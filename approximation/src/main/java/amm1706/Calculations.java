...
public class Calculations {
    ...
    protected static TreeMap<Double, Double> calcM(double u, double t, BigDecimal c, BigDecimal incC, BigDecimal cEnd, double v, double M, double D2) {
    ...
    protected static TreeMap<Double, Double> calcE(double u, double t, BigDecimal c, BigDecimal incC, BigDecimal cEnd, double v, double M, double D2, Type type) {
        ...
        while (cEnd.compareTo(c) >= 0) {
            ...
            double tmp4 = Math.exp(((2 * (u + valC * v)) / (valC * valC * D2)) * (1 - valC * M)) * normal(-(Math.sqrt(u + valC * v)) / (valC * Math.sqrt(D2 * x1)) * (x1 * (1 - valC * M) + 1));
            double tmp = (tmp1 + tmp2) - (tmp3 + tmp4);
            double Cf = 0, Cs = 0, Et, Et3, Dt, Ey, Ey3, Dy;
