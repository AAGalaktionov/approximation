...
import amm1706.Distributions.ExpExp;
import amm1706.Distributions.Type;
...
public class Simulation {
    static private ArrayList<Double> T = new ArrayList<>();
    static private ArrayList<Double> Y = new ArrayList<>();

    private static void randomator(ArrayList<Double> T, ArrayList<Double> Y, double t, Type nameMethod, double c, double u) {
    ...
    private static boolean crossing(TreeMap<Integer, LinkedHashMap<Double, Double>> Beams, ArrayList<Double> T, ArrayList<Double> Y, double u, double c, double t, double cForBeams, Type nameMethod) {
        T.clear();
        Y.clear();
        ...
        if (c == cForBeams && Beams.size() < 50) {
            ...
            while (((sumY - sumT * c) < u) && sumT < t && T.size() > count) {
                sumT += T.get(count);
                if (sumT > t) break;
                if (treeMap.containsKey(sumY - c * sumT))
                    treeMap.put(sumY - c * sumT + 0.001, sumT);
                else
    ...
    protected static void simulation(TreeMap<Double, Double> pSimulation, TreeMap<Integer, LinkedHashMap<Double, Double>> Beams, double c, double u, double t, double beamsCount, double cForBeams, Type nameMethod) {
        int countCross = 0;
